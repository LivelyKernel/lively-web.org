...
sudo find $mailman_data/lists/ -iname config.pck -exec chgrp www-data {} \;
sudo find $mailman_data/lists/ -maxdepth 1 -type d -exec chmod a+rwx {} \;

docker run \
  -e RELAY_DOMAINS=lists.lively-web.org \
  -e MAILMAN_DOMAIN=lists.lively-web.org \
  -p 25:25 -p 8081:80 \
  -v "$mailman_data/archives":/var/lib/mailman/archives \
  -v "$mailman_data/lists":/var/lib/mailman/lists \
  -v "$mailman_data/data":/var/lib/mailman/data \
